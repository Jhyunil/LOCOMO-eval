...
class RAGManager:
    ...
    def generate_response_origin(self, question, context):
        ...
        # Save the messages to a file for using in another LLM models
        # with open("prompt_start.txt", "w") as f:
        #     json.dump(messages, f, indent=4)
        # messages = "\n".join([f"{msg['role']}: {msg['content']}" for msg in messages])
        # with open("prompt_remind.txt", "w") as f:
        #     f.write(messages)
    ...
    def process_all_conversations(self, output_file_path):
        ...
        for key, value in tqdm(data.items(), desc="Processing conversations"):
            ...
            # Full_Conv[key].append({"context": chunks[0]})
            # with open("results/conv_context.jsonl", "w+") as f:
            #     json.dump(Full_Conv, f, indent=4)
